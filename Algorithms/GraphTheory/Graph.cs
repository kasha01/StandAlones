...
namespace Algorithms.GraphTheory
{
	public class Graph<T>
	{
		...
		// Using Recurssion - http://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/
		public void DFS(Node<T> n)
		{
			n.visited = true;
			Console.WriteLine(n.data);

			foreach (var i in n.adjNodes)
			{
				if (i.visited == false)
				{
					DFS(i);
		...
		Stack<Node<T>> topo_sort_nodes = new Stack<HelpersDataStructures.Node<T>>();
		public void Topological_Sort_DFS(Node<T> n)
		{
			n.visited = true;
			...
			// the last visited node will be pushed at the bottom of the stack
			topo_sort_nodes.Push(n);
		}
		...
		// O(E log V) => union of V nodes is log V, repeated E times.
		...
		// O(E log V) => decrease priority in min-queue is log V, repeated by number of adj nodes (E) => E Log(V)
